...
router.post("/staff_list_read", function (req, res) {
    ...
    ManagerModel.findAll({where: {id: {[Op.between]: [req.body.key, req.body.key + req.body.count - 1]}}, order: [["id", "ASC"]]}).then((models) => {
        ...
        res.send(models.map(v => [v.id, v.name, v.role, v.createdAt && v.createdAt.toLocaleDateString("default", {day: "numeric", month: "numeric", year: "numeric", hour: "numeric", minute: "numeric", second: "numeric"})]));
    }).catch((err) => {
