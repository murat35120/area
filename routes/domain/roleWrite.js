...
router.post("/role_write", function (req, res) {
    ...
    RoleModel.findOne({where: {name: req.body.title}}).then(async (model) => {
        if(!model) model = await RoleModel.create({name: req.body.title, list: JSON.stringify(req.body.rights)});
